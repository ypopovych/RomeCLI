...
func startOperation(operation: Operation?, parameters: [String]?) {
    ...
    case .ListClients:
        ClientCommand().list()
    case .AddClient:
        if let parameters = parameters, let clientName = parameters.first {
            print("Adding Client with name:", clientName)
            ClientCommand().add(name: clientName)
        } else {
            print("Error executing command, format should be: romecli -o add-client -p ClientName")
            return
        }
    case .DeleteClient:
        if let parameters = parameters, let clientId = parameters.first {
            print("Deleting Client with id:", clientId)
            ClientCommand().delete(id: clientId)
        } else {
    ...
    case .ListAssets:
        AssetCommand().list()
    case .GetAsset:
        if let parameters = parameters, let assetIdentifier = parameters.first {
            if parameters.count > 1 {
                let assetRevision = parameters[1]
                AssetCommand().getLatestByRevison(name: assetIdentifier, revision: assetRevision)
            } else {
                AssetCommand().getById(id: assetIdentifier)
            }
        } else {
    ...
    case .AddAsset:
        if let parameters = parameters, let assetName = parameters.first {
            let assetRevision = parameters[1]
            let assetPath = parameters[2]
            AssetCommand().addAsset(name: assetName, revision: assetRevision, path: assetPath)
        } else {
            print("Error executing command, format should be: romecli -o add-asset -p AssetName 1.0 /Users/alex/asset.zip")
            return
        }
    case .DeleteAsset:
        if let parameters = parameters, let assetId = parameters.first {
            print("Deleting Asset with id:", assetId)
            AssetCommand().delete(id: assetId)
        } else {
            print("Error executing command, format should be: romecli -o delete-asset -p 5724c65da9e0520f5aa25984")
            return
        }
    case .UpdateAsset:
        if let parameters = parameters, let assetId = parameters.first, let assetActive = parameters[1].boolValue() {
            print("Updating Asset with id:", assetId, "to", assetActive.description)
            AssetCommand().update(id: assetId, active: assetActive)
        } else {
...
do {
    ...
    if (help.value) {
        ...
        startOperation(operation: operation.value, parameters: parameters.value)
    }
} catch {
